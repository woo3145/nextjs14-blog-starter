...
- [깃허브](https://github.com/woo3145/flutter_vertical_heatmap)
...
Bottom-up 방식으로 개발을 진행했습니다.

1. 한칸을 표시하는 `HeatMapCell` - background/selected color, margin, radius 등 UI 요소와 기능을 위한 onTap, dateTime을 받아 onTap(dateTime)으로 실행하는 기능

2. 한 줄을 표시하는 `HeatMapRow` - 시작일과 표시할 칸 수를 받아 적절하게 cells과 emptySpaces를 표시하고 monthText를 선택적으로 받아 월을 표시

3. 여러줄을 표시하는 `HeatMap` - 시작일과 마지막일을 받아 7개씩 split하고 1일이 포함되면 monthText를 포함시켜 Row Widget List를 만들어 렌더링
